...
{
	{
		for (int i = 0; i < 10; ++i)
		{
			for (int j = 0; j < 10; ++j)
			{
				...
				modelMatrix = glm::translate(modelMatrix, glm::vec3(i + 5, 0, - j));
...

	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab9"], modelMatrix, mapTextures["earth"], NULL, true);
	}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2, bool rotateTexture)
{
	...
	GLint current_time = glGetUniformLocation(shader->program, "Time");

	if (rotateTexture) {
		glUniform1f(current_time, (GLfloat)Engine::GetElapsedTime());
	}
	else {
		glUniform1f(current_time, 0);
	}
	// Draw the object
...
Texture2D* Laborator9::CreateRandomTexture(unsigned int width, unsigned int height)
{
	...
	for (size_t i = 0; i < size / 4; i+=3) {
		data[i] = 255;
		data[i + 1] = 0;
		data[i + 2] = 0;
	}

	for (size_t i = size / 4; i < size / 2; i+=3) {
		data[i] = 0;
		data[i + 1] = 255;
		data[i + 2] = 0;
	}

	for (size_t i = size / 2; i < 3 * size / 4; i+=3) {
		data[i] = 0;
		data[i + 1] = 0;
		data[i + 2] = 255;
	}

	for (size_t i = 3 * size / 4; i < size; i+=3) {
		data[i] = 255;
		data[i + 1] = 255;
		data[i + 2] = 0;
	}
