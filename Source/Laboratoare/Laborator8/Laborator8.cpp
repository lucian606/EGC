...
int typeOfLight = 0;
float angle = 30; //Bonus pentru modificarea unghiului de iluminare tasta O-scade unghiul si tasta P-creste unghiul
using namespace std;
...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	GLint light_type = glGetUniformLocation(shader->program, "is_spot");
	glUniform1i(light_type, typeOfLight);

	GLfloat light_angle = glGetUniformLocation(shader->program, "angle");
	glUniform1f(light_angle, angle);
